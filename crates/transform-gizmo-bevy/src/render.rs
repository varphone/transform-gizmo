...
fn init_transform_gizmo_pipeline(render_world: &mut World) {
    let mesh_pipeline = render_world.resource::<MeshPipeline>().clone();
    render_world.insert_resource(TransformGizmoPipeline { mesh_pipeline });
}
    ...
    for (view_entity, view, _render_layers) in &views {
        ...
        for (entity, handle) in &transform_gizmos {
            ...
            transparent_phase.add_transient(Transparent3d {
                sorting_info: bevy_core_pipeline::core_3d::TransparentSortingInfo3d::AlwaysOnTop,
